...
const i18n = {
  en: {
    title: "SwiftCivic", reportIssue: "Report Issue", trackIssues: "Track Issues", myHistory: "My History", welcome: "Welcome", logout: "Logout", login: "Login", register: "Register", makeCity: "Make Your City", better: "Better", desc: "Report local infrastructure issues directly to your government administration. Pinpoint the exact location on the map and help improve your community.", issuesFixed: "Issues Fixed", avgResponse: "Avg Response", reportNew: "Report a New Issue", issueTitle: "Issue Title", category: "Category", uploadMedia: "Upload Media", description: "Description", location: "Location Pinpoint", submit: "Submit Report", submitting: "Submitting...", offlineAlert: "Saved Offline! Will sync when connected."
  },
  hi: {
...
const UserHistory = ({ onActivity }) => {
  ...
  const fetchMyIssues = async ({ showSpinner = false } = {}) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/user/my-issues`, {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/issues/${editingIssue.id}`, {
        method: 'PUT',
        headers: {
  ...
  useEffect(() => {
    if (formData.lat && formData.lng && formData.category && currentView === 'report') {
      const checkNearby = async () => {
        ...
        try {
          const res = await fetch(`${API_URL}/api/issues/check-duplicates?lat=${formData.lat}&lng=${formData.lng}&category=${formData.category}`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
          const data = await res.json();
      ...
      const fetchSuggestions = async () => {
        try {
          const res = await fetch(`${API_URL}/api/smart-suggestions?lat=${formData.lat}&lng=${formData.lng}&radius=1`);
          const data = await res.json();
