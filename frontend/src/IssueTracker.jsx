...
export default function IssueTracker() {
  ...
  useEffect(() => {
    ...
    // Fetch city ranking
    fetch(`${API_URL}/api/city-ranking`)
      .then(res => res.json())
  ...
  const handleUpvote = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`${API_URL}/api/issues/${id}/vote`, { 
        method: 'POST',
  ...
  const handleComment = async (id, e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`${API_URL}/api/issues/${id}/comments`, { 
        method: 'POST', 
  ...
  const handleVerify = async (id, type) => {
     try {
       const token = localStorage.getItem('token');
       const res = await fetch(`${API_URL}/api/issues/${id}/verify`, { 
         method: 'POST', 
         headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, 
    ...
    return (
      <div className="flex flex-col items-end gap-1">
        ...
        <div className="flex gap-1">
          {isOverdue && (
             <span className="bg-red-500 text-slate-900 text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded animate-pulse">⚠️ Delayed</span>
          )}
          {issue.is_escalated === 1 && (
             <span className="bg-indigo-600 text-slate-900 text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded shadow-lg border border-indigo-400">⚖️ Escalated</span>
          )}
        </div>
      </div>
    );
      ...
      <div className="flex-1 animate-in slide-in-from-left duration-500">
        <div className="flex justify-between items-end mb-8 border-b border-slate-300 pb-4">
          <div>
            ...
            <p className="text-slate-600 text-[10px] font-black uppercase tracking-[0.3em] mt-2">Real-time Community Infrastructure Tracking</p>
          </div>
        </div>

        <div className="flex flex-col gap-6 pb-12">
          {filteredIssues.map(issue => (
            <div key={issue.id} className={`glass-card p-6 flex flex-col gap-4 group relative ${issue.is_emergency ? 'ring-2 ring-red-500/50' : 'border-slate-200'}`}>
              
              {issue.is_emergency === 1 && (
                <div className="absolute top-[-12px] left-4 bg-red-600 text-slate-900 text-[10px] font-black uppercase tracking-[3px] px-3 py-1 rounded-full shadow-lg z-10 animate-pulse">
                  🚨 Emergency Report
                </div>
              )}

              <div className="flex justify-between items-center bg-slate-50/50 p-3 rounded-lg border border-slate-300/50">
                <span className="text-xs font-mono text-slate-600">ID: {issue.complaint_id}</span>
                {getStatusBadge(issue)}
              </div>

              <div className="grid md:grid-cols-[1fr_200px] gap-4">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 group-hover:text-primary transition-colors">{issue.title}</h3>
                  <p className="text-slate-600 text-sm mt-1 leading-relaxed">{issue.description}</p>
                  ...
                  {issue.admin_remarks && (
                    <div className="mt-4 bg-primary/5 border-l-4 border-primary p-3 rounded-r-lg">
                      ...
                      <p className="text-xs text-slate-700 italic">"{issue.admin_remarks}"</p>
                    </div>
                  )}
                </div>
                {(issue.media_url || issue.resolution_media_url) && (
                  <div className="flex flex-col gap-2">
                    <div className="text-[10px] text-slate-600 uppercase font-black tracking-widest text-center">Visual Proof</div>
                    <div className={`grid ${issue.resolution_media_url ? 'grid-cols-2' : 'grid-cols-1'} gap-1 rounded-lg overflow-hidden border border-slate-300`}>
                      {issue.media_url && (
                      ...
                      {issue.resolution_media_url && (
                        <div className="relative group/img cursor-zoom-in">
                          ...
                          <div className="absolute inset-x-0 bottom-0 bg-emerald-600/80 text-[8px] text-slate-900 text-center py-0.5 opacity-0 group-hover/img:opacity-100 transition-opacity uppercase font-bold">Resolved</div>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex justify-between items-center pt-4 border-t border-slate-300/50">
                <div className="flex gap-2">
                  <span className="text-[10px] text-slate-600 uppercase tracking-widest bg-white px-3 py-1.5 rounded-full border border-slate-300">{issue.category}</span>
                  <span className={`text-[10px] uppercase tracking-widest px-3 py-1.5 rounded-full border ${issue.priority === 'High' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-white text-slate-600 border-slate-300'}`}>{issue.priority} Priority</span>
                </div>
                
                <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-300/50 w-full">
                   {user && user.id !== issue.user_id && (
                     <div className="flex gap-2">
                        <button 
                          onClick={() => handleVerify(issue.id, 'exists')}
                          className="text-[9px] font-black uppercase tracking-widest bg-white text-amber-500 hover:bg-amber-500 hover:text-black px-4 py-2 rounded-lg border border-amber-500/20 transition-all flex items-center gap-2"
                        >
                          📍 Still Here? (+5 pts)
                        </button>
                        <button 
                          ...
                          className={`text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-lg border transition-all flex items-center gap-2 ${
                            issue.status === 'Resolved' 
                              ? 'bg-white text-red-400 hover:bg-red-500 hover:text-slate-900 border-red-500/20' 
                              : 'bg-white text-emerald-400 hover:bg-emerald-500 hover:text-black border-emerald-500/20'
                          }`}
                        >
                   ...
                   <div className="flex-1 flex justify-end">
                    <button 
                      ...
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition-all border ${user ? 'text-slate-700 hover:text-emerald-400 bg-white hover:bg-slate-200 border-slate-300 active:scale-95' : 'text-slate-600 bg-slate-50 border-slate-200 cursor-not-allowed'}`}
                    >
              ...
              <div className="mt-2 bg-slate-950 p-4 rounded-xl shadow-inner border border-slate-900">
                <div className="space-y-3 mb-4 max-h-40 overflow-y-auto pr-2 scrollbar-thin">
                  {(issue.comments || []).map((c, idx) => (
                    <div key={idx} className="bg-slate-50/50 p-2.5 rounded-lg border border-slate-200 flex justify-between items-start">
                      <div>
                        <p className="text-[10px] font-black text-primary uppercase mb-1">{c.author_name || c.author || 'Citizen'}</p>
                        <p className="text-xs text-slate-700">{c.content || c.text}</p>
                      </div>
                ...
                {user ? (
                  <form onSubmit={e => handleComment(issue.id, e)} className="flex gap-2">
                    <input 
                      ...
                      className="flex-1 bg-slate-50 border-slate-200 text-xs py-2 rounded-lg px-4 text-slate-900 placeholder-slate-600 focus:ring-1 focus:ring-primary outline-none transition-all" 
                    />
                  ...
                  <div className="text-center py-2 bg-slate-50/30 rounded-lg border border-slate-200/50">
                    <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest">Sign in to join the discussion</p>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="w-full lg:w-80 flex flex-col gap-6 animate-in slide-in-from-right duration-500 pt-8 lg:pt-16">
          ...
          <div className="glass-card p-6 border-emerald-500/20 bg-gradient-to-br from-slate-900 to-emerald-950/20">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-700">Cleanliness</span>
              ...
              <div className="text-center mt-4">
                <span className="text-xs text-slate-600">Rank #{cityRanking.rank}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="glass-card p-6 border-indigo-500/20 bg-gradient-to-br from-slate-900 to-indigo-950/20">
          ...
          <div className="space-y-4">
            {leaderboard.map((user, idx) => (
              <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors">
                <div className="flex items-center gap-3">
                  <span className={`w-6 h-6 rounded flex items-center justify-center text-[10px] font-black ${idx === 0 ? 'bg-amber-500 text-black' : idx === 1 ? 'bg-slate-300 text-black' : idx === 2 ? 'bg-orange-500 text-slate-900' : 'bg-slate-200 text-slate-600'}`}>
                    {idx + 1}
                  </span>
            ...
            {leaderboard.length === 0 && <p className="text-xs text-slate-600 text-center italic">No contenders yet</p>}
          </div>
          <div className="mt-6 pt-4 border-t border-white/5 text-center">
            <p className="text-[10px] text-slate-600 mb-2 uppercase tracking-tighter">Your contribution matters</p>
            <div className="text-[9px] text-slate-600 leading-tight">Resolve issues to climb the city's civic leaderboard and earn prestige.</div>
          </div>
        </div>

        <div className="glass-card p-6 border-amber-500/20">
          <h4 className="text-xs font-black text-slate-600 uppercase tracking-[3px] mb-4">Community KPI</h4>
          <div className="flex justify-between items-center mb-1">
            <span className="text-xs text-slate-600 font-bold">Resolution Rate</span>
            <span className="text-xs text-emerald-400 font-black">78%</span>
          </div>
          <div className="w-full h-1.5 bg-white rounded-full overflow-hidden">
            <div className="h-full bg-emerald-500 w-[78%]"></div>
          </div>
          <p className="text-[9px] text-slate-600 mt-3 italic line-clamp-2 italic leading-relaxed font-medium">Monitoring the success of civic collaboration across the metropolitan area.</p>
        </div>
      </div>
    </div>
  );
