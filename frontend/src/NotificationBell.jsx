  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        className="relative text-slate-600 hover:text-slate-900 transition-colors duration-200 outline-none group flex items-center justify-center p-2 rounded-lg hover:bg-white"
        title="Notifications"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-white/95 backdrop-blur-xl border border-slate-300 shadow-2xl rounded-xl overflow-hidden z-[100] animate-in slide-in-from-top-2 origin-top-right">
          <div className="p-4 border-b border-slate-300 bg-slate-50/50 flex justify-between items-center">
            <h3 className="font-bold text-slate-900 tracking-widest uppercase text-xs">Notifications</h3>
          ...
          <div className="max-h-72 overflow-y-auto">
            {notifications.length === 0 ? (
              ...
              <div className="flex flex-col">
                {notifications.map(n => (
                  <div key={n.id} className="p-4 border-b border-slate-300/50 hover:bg-slate-200/50 transition-colors cursor-pointer group">
                    <p className="text-sm text-slate-700 group-hover:text-slate-900 transition-colors">{n.message}</p>
          ...
          {notifications.length > 0 && (
            <div className="p-3 bg-slate-50/50 text-center border-t border-slate-300">
              <button 
